    ...
    switch(File){
        case 'PDF' :
            if(PeriodID == "" || AgentID == ""){
                ...
                window.open("report.sell.3digi.pdf.php");
            }
            break;
        default:
            // Send error to user
...
function DisplayTable3DigiList(){
    var PeriodID = document.getElementById('PeriodID').value;
    var AgentID = document.getElementById('AgentID').value;
    $.ajax({
        type: "POST",
        url: "view/report.sell.3digi.php",
        data: "CRUD=Read&View=display-table-3digi-list&PeriodID="+PeriodID+"&AgentID="+AgentID,
        success: function(Result){
			...
			$('#Totals').val(Totals);
            $("#display-table-3digi-list").html(result[0]);
            $('#table-3digi-list').DataTable({
                "dom"           : "Bfrtip",
                "autoWidth"     : false,
                "responsive"    : true,
                "processing"    : true,
...
function DetailSend3Digi(PeriodID, AgentID, Numbers){
    $.ajax({
        type: "POST",
        url: "view/report.detail.sell.3digi.php",
        data: "CRUD=Read&Numbers="+Numbers+"&PeriodID="+PeriodID+"&AgentID="+AgentID,
        success: function(Result){
            $("#modal-body").html(Result);
            $('#table-detail-3digi-list').DataTable({
                "autoWidth"     : false,
                "responsive"    : true,
                "processing"    : true,
                "bInfo"         : false,
